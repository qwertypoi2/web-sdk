...
// Searching for the file in common locations
const paths = ['src/GameGrid.svelte', 'GameGrid.svelte', 'apps/lofi_library/src/GameGrid.svelte'];
let targetPath = paths.find(p => fs.existsSync(p));

if (!targetPath) {
  console.error("❌ Could not find GameGrid.svelte. Please ensure you are in the lofi_library folder.");
  process.exit(1);
}
...
console.log("✅ SUCCESS: Found file at " + targetPath + " and overwrote it with the updated code.");
