...
const content = `<script>
  ...
  const betOptions = [0.10, 0.20, 0.50, 1.00, 2.00, 5.00];
...
<div class='game-container'>
  ...
  {#if showWinBanner}
    <div class='banner' in:scale out:fade>
      ...
      <div class='details'>
        {#each activeClusters as c}
          <div class='item'>
            <div class='icon-circle'><img src={'/' + c.image} alt='' /></div>
            <span>x{c.count}</span>
          </div>
        {/each}
      </div>
...
<style>
  ...
  .banner { position:absolute; top:40%; left:50%; transform:translate(-50%, -50%); padding:15px 20px; border-radius:24px; z-index:5000; text-align:center; color:white; background:#e07a5f; border:4px solid white; box-shadow:0 15px 45px rgba(0,0,0,0.4); min-width:220px; }
  .amt { font-size:1.6rem; font-weight:900; margin-bottom: 5px; }
  .details { display:flex; justify-content:center; gap:10px; flex-wrap:wrap; margin-bottom: 8px; }
  .item { display:flex; align-items:center; gap:5px; background:rgba(0,0,0,0.2); padding:2px 8px; border-radius:15px; font-weight:bold; font-size:0.9rem; }
  .icon-circle { width:22px; height:22px; background:white; border-radius:50%; display:flex; align-items:center; justify-content:center; padding:2px; }
  ...
  .stats { display:flex; justify-content:space-between; font-size:0.85rem; font-weight:bold; margin-bottom:10px; }
  ...
  .rtp-tag { background:#f2cc8f; color:#3d405b; font-weight:bold; font-size:0.7rem; padding:4px; border-radius:5px; margin-bottom:15px; }
  .pay-grid { display:grid; grid-template-columns:1fr 1fr; gap:8px; margin:15px 0; font-size:0.8rem; }
  .pay-item { display:flex; justify-content:space-between; border-bottom:1px solid #eee; padding-bottom:2px; }
  .bet-grid { display:grid; grid-template-columns: 1fr 1fr 1fr; gap:8px; margin-top:10px; }
  .bet-grid button { background:#81b29a; border:none; color:white; padding:10px; border-radius:8px; font-weight:bold; }
  .close-btn { background:#3d405b; color:white; border:none; padding:10px; border-radius:8px; width:100%; margin-top: 10px; cursor:pointer; font-weight:bold; }
</style>`;

fs.writeFileSync(filePath, content);
console.log('✅ RESTORED LOFI GUI');
