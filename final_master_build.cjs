...
const content = `<script>
  ...
  async function spin() {
    ...
    if (!bgMusic.playing && !mute) bgMusic.play().catch(() => {});
    isSpinning = true;
    showWinBanner = false;
...
<div class='game-container'>
    ...
    <div class='banner' in:scale out:fade>
      ...
      <div class='details'>
        {#each activeClusters as c}
          <div class='item'>
            <div class='icon-frame'><img src={'/' + c.image} alt='' /></div>
            <span>x{c.count}</span>
          </div>
        {/each}
      </div>
  ...
  <div class='panel'>
    ...
    <div class='controls'>
      ...
      <button class='side-btn' on:click={() => { mute = !mute; mute ? bgMusic.pause() : bgMusic.play(); }}>{mute ? '🔇' : '🔊'}</button>
      <button class='spin-btn' on:click={spin}>{isSpinning ? '...' : (autoSpinsRemaining > 0 ? autoSpinsRemaining : 'SPIN')}</button>
...
<div class='modal-backdrop' on:click|self={() => {showBetModal = false; showAutoModal = false; showInfoModal = false;}}>
  <div class='lofi-modal'>
    ...
    {#if showInfoModal}
      ...
      <div class='info-text'>
        <p>Connect <b>5+ matching pictures</b> touching side-by-side or up-and-down!</p>
        <p>Winning groups <b>POP</b>! New pictures fall in, and every chain reaction makes your <b>Multiplier</b> grow!</p>
      </div>
...
<style>
  ...
  .item { display:flex; align-items:center; gap:6px; background:rgba(0,0,0,0.3); padding:4px 10px; border-radius:15px; font-weight:bold; font-size:0.9rem; }
  .icon-frame { width:28px; height:28px; background:rgba(61, 64, 91, 0.85); border:1.5px solid white; border-radius:50%; display:flex; align-items:center; justify-content:center; padding:3px; }
  ...
  .lofi-modal { background:#fdfaf5; padding:25px; border-radius:20px; width:85%; max-width:320px; text-align:center; border: 4px solid #3d405b; }
  ...
  .rtp-tag { font-size: 0.65rem; color: #999; margin-top:10px; font-style:italic; }
...
console.log('✅ MASTER BUILD COMPLETE: Audio, Visuals, and Mobile Handling optimized.');
