...
const targetPath = findFile(process.cwd(), 'GameGrid.svelte');

if (!targetPath) {
    console.error("❌ Could not find GameGrid.svelte anywhere in this folder!");
    process.exit(1);
}
...
const newContent = \`<script>
  ...
  async function processRound() {
    ...
    if (wins.size > 0) {
      ...
      balance += winAmt;
      activeClusters = clusterDetails.map(c => ({
        image: symbols.find(s => s.id === c.id).image,
        count: c.count
      }));
      isDimmed = true;
      showWinBanner = true;
  ...
  function findWins() {
    ...
    for (let i = 0; i < 25; i++) {
      ...
      while (queue.length > 0) {
        ...
        [curr-5, curr+5, curr-1, curr+1].forEach(n => {
          ...
          if ((side || updown) && !visited.has(n) && grid[n]) {
            if (grid[n].id === typeId || grid[n].id === 'Wild') { 
      ...
      if (cluster.length >= 5) {
        cluster.forEach(idx => { wins.add(idx); if (grid[idx].id !== 'Wild') processedIndices.add(idx); });
    ...
    <div class="banner" in:scale out:fade>
      ...
      <div class="details">
        {#each activeClusters as cluster}
          <div class="item">
            <div class="icon-wrap"><img src="/{cluster.image}" alt="win symbol" /></div>
            <span>x{cluster.count}</span>
          </div>
        {/each}
      </div>
...
<div class="modal" on:click|self={() => showInfoModal = false}>
  <div class="box">
    <div style="background:#f2cc8f; color:#3d405b; font-weight:bold; font-size:0.75rem; padding:6px; border-radius:5px; margin-bottom:15px; border:1px solid #3d405b;">Theoretical RTP: 96.45%</div>
    <h3 style="margin-top:0">HOW TO WIN 🧩</h3>
    <div class="pay-grid">
      {#each symbols.filter(s => !s.id === 'Wild') as s}
...
<style>
  ...
  .banner { position:absolute; top:42%; left:50%; transform:translate(-50%, -50%); padding:20px; border-radius:24px; z-index:5000; text-align:center; color:white; background:#3d405b; border:4px solid #e07a5f; box-shadow:0 15px 45px rgba(0,0,0,0.5); width: 85%; }
  .amt { font-size:1.8rem; font-weight:900; color: #f2cc8f; margin-bottom: 8px; }
  .details { display:flex; justify-content:center; gap:10px; flex-wrap:wrap; margin-bottom:10px; }
  .item { display:flex; align-items:center; gap:6px; background:rgba(255,255,255,0.1); padding:4px 10px; border-radius:15px; font-weight:bold; }
  .icon-wrap { width:24px; height:24px; background:white; border-radius:50%; display:flex; align-items:center; justify-content:center; padding:2px; }
  .icon-wrap img { width:100%; height:100%; object-fit:contain; }
  .mult { font-size:0.85rem; font-weight:900; color:#e07a5f; background: rgba(0,0,0,0.2); padding: 2px 10px; border-radius: 10px; }
  ...
  .box { background:white; padding:20px; border-radius:15px; width:85%; max-width:300px; text-align:center; position: relative; border: 3px solid #3d405b; }
  ...
  .close { background:#3d405b; color:white; border:none; padding:10px; border-radius:8px; width:100%; margin-top: 10px; cursor:pointer; font-weight:bold; }
</style>\`;

fs.writeFileSync(targetPath, newContent);
console.log("✅ SUCCESS: Found file at " + targetPath + " and forced update.");
